    ...
    <main class="w-full max-w-7xl anim-row anim-row-1">
        ...
        <nav class="flex gap-12 border-b border-white/10 font-sans text-[11px] uppercase tracking-[0.1em] mb-16">
            <button 
                ...
                class="pb-4 transition-all duration-100 cursor-pointer {activeTab === 'gallery' ? 'text-white border-b border-white' : 'text-white/30 hover:text-white'}">
                Collection
            </button>
            <button 
                onclick={() => activeTab = 'research'}
                class="pb-4 transition-all duration-100 cursor-pointer {activeTab === 'research' ? 'text-white border-b border-white' : 'text-white/30 hover:text-white'}">
                De Hooch Research
            </button>
        </nav>
                                    ...
                                    <div class="group grid grid-cols-[3px_1fr] gap-6 items-start">
                                        ...
                                        <div>
                                            <h4 class="font-serif text-[18px] text-white/90 leading-snug">
                                                {#if bib.url}
                                                    <a href={bib.url} target="_blank" class="hover:text-white transition-colors duration-100 italic underline decoration-white/10 underline-offset-4 hover:decoration-white/40">
                                                        {bib.title}
                                                    </a>
                                                {:else}
                        ...
                        {#if footnoteVisible}
                            <div in:fade={{ duration: 200 }} class="border border-white/10 bg-white/2.5 p-6 space-y-4">
                                ...
                                <div class="font-sans text-[13px] text-white/50 leading-relaxed space-y-3">
                                    ...
                                    <p>
                                        The supposed “failure” or decline of De Hooch’s art was often blamed on the artist’s deteriorating mental health since he was thought to have died in an asylum in 1684. The fact that the mental health angle is incorrect complicates the picture, and it is now more understood that the artist's decline was actually a shift toward refined, classicizing elements that paved the way for 18th century Dutch painting.
                                    </p>
                                </div>
                            </div>
                        {/if}
                ...
                <div class="space-y-8">
                    ...
                    <section>
                        ...
                        <div class="space-y-10">
                            {#each sortedBibliography as bib (bib.title)}
                                <div class="group grid grid-cols-[3px_1fr] gap-6 items-start">
                                    <div class="w-px h-full bg-white/8 group-hover:bg-white/25 duration-75 transition-colors mt-1 self-stretch"></div>
                                    <div>
                                        <h4 class="font-serif text-[18px] text-white/90 leading-snug">
                                            {#if bib.url}
                                                <a href={bib.url} target="_blank" class="hover:text-white transition-colors duration-75 italic underline decoration-white/10 underline-offset-4 hover:decoration-white/40">
                                                    {bib.title}
                                                </a>
                                            {:else}
                                                <span class="italic">{bib.title}</span>
